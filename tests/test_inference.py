...
def test_openai_client_image_methods(monkeypatch, capsys):
    ...
    with patch("openai.resources.images.Images.edit", side_effect=raise_error) as mock_edit:
        ...
        for method_name in method_names:
            ...
            with (
                tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_img1,
...
def test_openai_client_animation_suggestion(monkeypatch):
    ...
    monkeypatch.setattr(
        inference.openai.responses,
        "create",
        lambda **kwargs: (_ for _ in ()).throw(Exception("err")),
    )
...
def test_ai_model_manager_and_missing_input(tmp_path, monkeypatch):
    ...
    ref_input_missing = inference.GenerateReferenceImageInput(
        output_folder="out", project_description="", keywords="", images=[], camera=None
    )
...
def test_ai_model_manager_requires_provider_configuration(tmp_path, monkeypatch):
    ...
    sfile.write_text(
        json.dumps({"Selected Inference Provider": "GOOGLEAI", "GOOGLE_AI_STUDIO_API_KEY": "key"})
    )
...
def test_googleai_client_image_generation_methods(tmp_path, monkeypatch):
    ...
    # Reference image
    ref_input = inference.GenerateReferenceImageInput(
        output_folder=str(tmp_path), project_description="pd", keywords="kw", images=[], camera=None
    )
...
def test_googleai_client_animation_suggestion(monkeypatch):
    ...
    monkeypatch.setattr(
        inference.genai, "Client", lambda api_key=None: (_ for _ in ()).throw(Exception("err"))
    )
    input_obj2 = inference.GenerateSpriteAnimationSuggestion(
        output_folder="o",
        animation_names=["a"],
...
def test_base_ai_client_pass_methods():
    ...
    assert (
        client.generate_description(inference.GenerateDescriptionInput(keywords=None, images=[]))
        is None
    )
    assert (
        client.generate_keywords(
            inference.GenerateKeywordsInput(project_description="desc", images=[])
        )
        is None
    )
    assert (
        client.generate_reference_image(
            inference.GenerateReferenceImageInput(
    ...
    assert (
        client.generate_base_sprite_image(
    ...
    assert (
        client.generate_sprite_animation_suggestion(
            inference.GenerateSpriteAnimationSuggestion(
                output_folder="out",
                animation_names=["a1"],
                sprite_description="spr",
                project_description="pd",
...
def test_googleai_client_generate_reference_image_error(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        inference.genai, "Client", lambda api_key=None: (_ for _ in ()).throw(Exception("ref_fail"))
    )
    client = inference.GoogleAIClient(api_key="key")
    input_obj = inference.GenerateReferenceImageInput(
        output_folder="out", project_description="pd", keywords="kw", images=[], camera=None
...
def test_googleai_next_sprite_image_open_error(mock_image_open, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(
        inference.genai, "Client", lambda api_key=None: DummyGClient(parts=None, text_parts=None)
    )
...
def test_googleai_next_sprite_api_error(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(
        inference.genai,
        "Client",
        lambda api_key=None: DummyGClient(fail_generate=True, fail_message=error_message),
    )
