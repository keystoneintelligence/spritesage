...
[`modelmanager` repository](https://github.com/keystoneintelligence/modelmanager)
at commit `453dfe709dddc028467862d123613ef5cb1a5975` on branch
`codex/local-qwen-prompt-enhancement`. `modelmanager-source.sha256.txt`
records every authored source file, and the wheel payload matches that commit.
...
Install Sprite Sage from a checkout with
`pip install --find-links vendor/wheels -e ".[dev]"`. The package is not yet
published on PyPI; a standalone registry-based `pip install spritesage` needs a
published Model Manager wheel or another configured package source.
